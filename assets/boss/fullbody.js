...
const EnemyFullbody=(()=>{
 ...
 function mask(p,w,h,profile){
  ...
  const white=profile.key==='white',limit=white?238:14;
  const seen=new Uint8Array(w*h),queue=new Int32Array(w*h);let head=0,tail=0;
  const background=n=>{const i=n*4,lo=Math.min(p[i],p[i+1],p[i+2]),hi=Math.max(p[i],p[i+1],p[i+2]);return white?lo>=limit&&hi-lo<=20:hi<=limit;};
  ...
  for(let n=0;n<w*h;n++)if(!seen[n]){
   ...
   const coverage=white?(255-lo)/35:(hi-8)/24;
   if(coverage<1&&(!white||hi-lo<20))p[i+3]=Math.round(p[i+3]*Math.max(0,coverage));
  }
 ...
 function get(skin){
  ...
  if(original){
   ...
   for(const p of parts){g.save();g.translate(p.x-x,p.y-y);if(p.flip){g.translate(p.w,0);g.scale(-1,1);}g.drawImage(p.img,0,0,p.w,p.h);g.restore();}
