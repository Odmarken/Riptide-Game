...
const TideMotion=(()=>{
 ...
 function pose(id,time,motion,phase,alive=true,stride=1){
  ...
  const step=Math.abs(Math.sin(phase)),hop=hoppers.has(id)?1.5:1;stride=clamp(stride,1,1.6);
  return {scaleX:1-breath*.006+(id==='duskmoth'?Math.sin(time*8+phaseFor(id))*.035:0),
   scaleY:1+breath*.016,lift:flying?.027+.017*Math.sin(p*.8):step*.026*walk*hop*stride,
   tilt:flying?Math.sin(p*.75)*.018:Math.sin(phase)*.023*walk*stride,walk:flying?0:walk};
 }
 ...
 function walkFrame(f,id,phase,strength,deviceHeight,stride=1){
  ...
  if(!amount)return f;
  stride=Math.round(clamp(stride,1,1.6)*10)/10;
  const key=ids.get(f)+':'+h+':'+step+':'+amount+':'+stride,cached=get(key);if(cached)return cached;
  ...
  for(let i=0;i<24;i++){
   ...
   if(!cw)continue;
   const lift=Math.pow(Math.max(0,Math.sin(cycle+(i+.5)/24*Math.PI*2)),1.4)*h*.027*amount*stride;
 ...
 function draw(g,f,id,x,y,height,fx,motion,phase,alpha,time,alive=true,stride=1){
  const p=pose(id,time,motion,phase,alive,stride),width=height*f.w/f.h;
  ...
  const m=g.getTransform(),deviceHeight=height*Math.hypot(m.c,m.d),paint=walkFrame(f,id,phase,p.walk,deviceHeight,stride);
