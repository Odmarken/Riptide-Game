...
function harness(gold=1000000){
 ...
 const c={TideFarm,Tides,Date,Math,performance:{now:()=>1234},
  S:{gold,overflow:0,scraps:1000,farm:{owned:true,b:[],c:[],r:[],lvl:1},tides:Tides.createCollection()},
  world:{w:8400,h:2600,solids:[]},hero:{x:1000,y:1020,r:16,dead:false},zone:{farm:true},gameOn:true,buildMode:false,
...
function buy(h,x=1000,y=1000){h.c.placeFarmItem(TideFarm.BUILDING_ID,x,y);h.el('farmCheckYes').onclick();return h.c.S.farm.b.at(-1);}
...
test('insufficient gold leaves the incubator ghost and wallet intact without creating a station',()=>{
 const h=harness(499999);h.c.placeFarmItem(TideFarm.BUILDING_ID,1000,1000);const before=clone(h.c.S);
...
test('station identity survives the actual move/flip/save repair routes while incubation stays attached',()=>{
 ...
 h.c.dropMoved(1300,900);assert.equal(it.x,1300);assert.equal(it.y,900);assert.equal(it._moving,undefined);assert.equal(it.breedingStationId,id);
...
test('a far click targets the actual mirrored door and the delayed open rechecks the current world',()=>{
 const h=harness(),it=buy(h);it.fl=-1;h.c.hero.x=200;h.c.hero.y=200;
...
test('mass delete checks incubation again on confirmation and preserves every selected item without refunds',()=>{
 const h=harness(),it=buy(h);h.c.S.farm.b.push({t:'tree_farm',x:1100,y:1000});h.c.S.farm.c.push({t:'hay',x:1150,y:1000});h.c.S.farm.r.push({t:'dirt_road',x0:900,y0:900,x1:1200,y1:1100});
 h.c.pendingDelRect={x0:800,y0:800,x1:1400,y1:1200};
