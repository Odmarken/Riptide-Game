...
test('before the signing the Hand can lay out a close, a season and what the loan will cost',()=>{
 ...
 assert.equal(v.loan,500000);assert.equal(v.line,650000);assert.equal(v.reserve,150000);assert.equal(v.closes,N);
 assert.equal(v.interest,Math.round(500000*E.LOAN_RATE));assert.equal(v.target,450000);assert.equal(v.due,50000);
 assert.ok(v.perCloseIn>0&&v.perCloseOut>v.perCloseIn,'the city does not cover its own costs');
 assert.ok(v.net<-4000,'and then there is the interest: '+v.net);
...
test('a season of doing nothing: the debt is called in, the grade is poor, the line narrows and the money gets dearer',()=>{
 ...
 assert.equal(v.swept,50000,'what was owed above the target came straight out of the strongroom');assert.equal(s.loan,450000);assert.equal(s.treasury,cash+last.net-50000);
 assert.ok(v.worth<v.worthStart,'the crown is worth less than it was');assert.ok(v.interest>=N*2400);
 assert.equal(s.limit,Math.round(650000*.95));assert.equal(s.rate,rate(E.LOAN_RATE+.0005));
 assert.equal(s.season.n,2);assert.equal(s.season.target,405000);assert.deepEqual(s.season.series,[]);assert.equal(s.reviewSeen,false);
 assert.ok(s.council.coin<60,'and the Master of Coin has an opinion');
});
...
test('a season that ends with the debt down, the strongroom never dry and the crown worth more earns an A: a wider line and cheaper money',()=>{
 ...
 season(s,(st,i)=>{if(i===N-1){E.deposit(st,120000,1e9,{});E.repay(st,50000);}});
 const v=s.seasons[0];
 assert.equal(v.grade,'A');assert.equal(v.paid,true);assert.equal(v.swept,0);assert.equal(v.reduced,50000);assert.equal(s.loan,450000);
 assert.equal(s.limit,Math.round(650000*1.12+50000));assert.equal(s.rate,rate(E.LOAN_RATE-.0005));
 assert.ok(s.trust>trust0+6,'the realm hears of it');
 /* in the black but no richer: a B, and a smaller reward */
 const b=open();season(b,(st,i)=>{if(i===N-1)E.repay(st,50000);});
 assert.equal(b.seasons[0].grade,'B');assert.equal(b.limit,Math.round(650000*1.06+50000*.75));
 /* it is where the debt ENDS that counts: gold sent to and fro earns nothing */
 const c=open();season(c,st=>{E.borrow(st,{},100000);E.repay(st,100000);});
 ...
 const d=open();season(d,(st,i)=>{if(i===0)E.borrow(st,{},150000);if(i===N-1)E.repay(st,200000);});
 ...
 const many=open();for(let k=0;k<12;k++)season(many,(st,i)=>{if(i===N-1){E.deposit(st,200000,1e9,{});E.repay(st,1e9);}});
...
test('a shortfall is covered from the line at a fee while there is a line',()=>{
 ...
 assert.equal(r.covered,-f.net);assert.equal(s.treasury,0);assert.equal(s.loan,500000-f.net+Math.round(-f.net*E.COVER_FEE));
 ...
 season(s,(st,i)=>{if(i===N-2){E.deposit(st,300000,1e9,{});E.repay(st,st.loan-st.season.target);}});
...
test('past the line the treasury is in the red: nothing may be raised or ordered, and after a close of grace the bailiffs take one thing at a close',()=>{
 ...
 const red=()=>{s.treasury=-60000;return E.tick(s,{},quiet);};
 const first=red();
 ...
 const t=open();t.loan=t.limit;t.works={university:{left:4}};t.treasury=-1000;E.tick(t,{},quiet);t.treasury=-1000;
 const net=E.forecast(t,{}).net;E.tick(t,{},quiet);
 assert.equal(t.treasury,-1000+net+Math.round(E.WORKS.find(w=>w.id==='university').cost*K*.5));
 ...
 s.treasury=400000;E.tick(s,{},quiet);
 ...
 const f=open();f.loan=f.limit;season(f,st=>{st.treasury=-60000;});
...
test('the Bank tab has everything it shows, and the season survives a save',()=>{
 const s=open();E.borrow(s,{},50000);for(let i=0;i<5;i++)E.tick(s,{},quiet);
 const v=E.bankView(s,{});
 assert.equal(v.chartered,true);assert.equal(v.loan,550000);assert.equal(v.limit,650000);assert.equal(v.room,100000);assert.equal(v.interest,Math.round(550000*s.rate));
 assert.equal(v.length,N);assert.equal(v.season.n,1);assert.equal(v.season.closes,5);assert.equal(v.season.left,N-5);assert.equal(v.season.toRepay,100000);
 assert.equal(v.season.series.length,5);assert.ok(v.season.series.every(p=>Number.isInteger(p.t)&&p.d===550000&&Number.isInteger(p.n)));
 ...
 const bad=E.normalize({v:E.VERSION,chartered:true,loan:300000,limit:'x',rate:-1,guards:99,seized:['quay','quay','ghost'],season:{n:'x',series:[{t:'5',d:-3},null]},seasons:[{grade:'Z'},{grade:'B',n:1,series:'no'}]});
