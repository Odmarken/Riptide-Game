...
function city(seed=13){
 ...
 vm.runInContext(section('const CATH_ART=','const CITY_FOOT=')+section('const CITY_NAMES=','/* 🧱 The floor of the City')+source.match(/^function mulberry32\(.*$/m)[0]+`;buildCity(mulberry32(${seed}));globalThis.folk=CITY_FOLK;globalThis.watch=CITY_WATCH;globalThis.spacing=WATCH_SPACING;`,context);
...
test('every stroll follows the streets, one lane wide, and keeps out of the nave, the gate and the well',()=>{
 for(const seed of [1,13,42,8675309]){
  ...
  const flight=w.rails.filter(r=>!r.harbor); /* 👑 the palace stair: nobody strolls between, or through, its balustrades */
  assert.equal(flight.length,4);
  const quayFlight=w.rails.filter(r=>r.harbor),flightTop=Math.min(...quayFlight.map(r=>r.y)); /* ⚓ nor down the harbour flight at the south end of the central avenue */
  assert.equal(quayFlight.length,6);
   ...
   for(let i=0;i<n.pts.length;i++){
    ...
    if(i){
     ...
     for(let t=0;t<=1;t+=1/16){
      ...
      assert.ok(p.y<flightTop-40||Math.abs(p.x-8400)>160,`${n.name} walks down the harbour flight`);
