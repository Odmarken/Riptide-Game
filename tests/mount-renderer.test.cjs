 ...
 for(const id of ['horse','leopard','spectral-tiger']){
  ...
  for(const phase of [Math.PI/4,Math.PI/2,Math.PI*5/4,Math.PI*3/2]){
   ...
   for(let i=0;i<4;i++){
    ...
    assert.equal(p[1]+p[3],pose.height,'the entire source sole is preserved');
    ...
    const root=point(upper.matrix,u[4],u[5]),pad=pose.ops[0].matrix[4];
    close(root[0],u[0]+pad,'upper leg stays attached horizontally');close(root[1],u[1],'upper leg stays attached vertically');
    ...
    assert.ok(sole[1]<=pose.height,'lift never pushes the original sole below frame');
   }
...
test('the rider hip follows the saddle through breathing, motion, scale and both facing directions',()=>{
 ...
 for(const id of ['horse','leopard','spectral-tiger'])for(const fx of [-1,1])for(const moving of [0,.5,1])for(const phase of [0,1,2,3]){
  ...
  const sx=(l.art.profile.seat[0]*l.art.iw-l.art.ground[0])*l.px;
  const sy=(l.art.profile.seat[1]*l.art.ih-l.art.ground[1])*l.px*l.bodyScaleY;
  close(l.riderX,fx*(sx*Math.cos(l.angle)-sy*Math.sin(l.angle)),'hip x at saddle');
  close(l.riderY+l.hipY,l.groundY+l.bob+sx*Math.sin(l.angle)+sy*Math.cos(l.angle),'hip y at saddle');
  assert.ok(Math.abs(l.bob)<=.48*1.3,'body bob does not float above its feet');
  assert.ok(Math.abs(l.riderAngle)<.03,'rider lean remains small');
